                ...
                else if (m_spellInfo->SpellFamilyFlags[1] & 0x1)
                {
                    ...
                    if (Player* caster = m_caster->ToPlayer())
                    {
                        ...
                        if (Item* item = caster->GetWeaponForAttack(RANGED_ATTACK))
                        {
                            ...
                            float dmg_min = weaponTemplate->Damage[0].DamageMin + weaponTemplate->Damage[1].DamageMin;
                            float dmg_max = weaponTemplate->Damage[0].DamageMax + weaponTemplate->Damage[1].DamageMax;
